...
def explain_gap_detection(kb: dict, gap_df: pd.DataFrame) -> list[dict]:
    ...
    for _, row in gap_df.iterrows():
        ...
        explanations.append({
            ...
            "evidence_source": (
                "Evidence strength + citation poverty"
                " + method consensus + literature coverage"
            ),
...
def explain_novelty(novelty_df: pd.DataFrame) -> list[dict]:
    ...
    for _, row in novelty_df.iterrows():
        ...
        if row.get("emerging_score", 0) > 0.5:
            novelty_source = "High emerging concept score"
        else:
            novelty_source = "Moderate topic saturation"
        explanations.append({
            "component": "novelty_scoring",
            "theme": theme,
            "evidence_source": (
                "Topic saturation + publication density"
                " + citation density + emerging concepts"
                " + recency"
            ),
            "confidence_score": score,
            "supporting_papers": [],
            "alternative_interpretations": [
                "Different embedding models may yield"
                " different saturation estimates",
                "Novelty is relative to the current corpus;"
                " adding papers may change classification",
            ],
            "limitations": [
                "Novelty scoring uses MiniLM embeddings"
                " which may miss domain-specific nuances",
                "Emerging concept detection is"
                " keyword-based, not semantic",
            ],
            "explainability_statement": (
                f"Novelty for '{theme}' is "
                f"{score:.2f} ({level}). "
                f"{novelty_source} "
...
def _generate_report(theme_explanations: list[dict],
    ...
    lines.append(
        ...
        "Evidence Source, Confidence Score, "
        "Supporting Papers, Alternative Interpretations, "
        "Limitations, and Explainability Statement.\n"
    )
    ...
    for category, explanations in [
        ...
        for exp in explanations[:5]:
            ...
            for lim in exp.get("limitations", []):
                lines.append(f"  - {lim}")
            lines.append(
                f"- **Explainability Statement:** "
                f"{exp.get('explainability_statement', '')}\n"
            )
