...
class OutputStore:
    ...
    def read_json(self, name: str) -> Any:
        """Read a JSON artifact from the run directory. Returns None if missing."""
        p = self.path(name)
        if not p.exists():
            return None
    ...
    def list_files(self, pattern: str = "*.json") -> list[str]:
        """List files matching a pattern in the run directory."""
        return [f.name for f in self.run_dir.glob(pattern)]
