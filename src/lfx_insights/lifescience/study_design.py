...
weighted blend of THREE inputs that must hold together â€” we never take an OR over
them, and in particular we never equate "many papers" with "mature":

  * ``breadth``      â€” how much evidence exists (normalized across themes AND against
                       an absolute target, so a single tiny theme still reads as low).
  * ``recency``      â€” share of dated papers within a recency window of the corpus
                       max year (a stale literature is not confirmatory-ready, and a
                       literature with no dated papers â€” unknown recency â€” fails the
                       gate too: we cannot vouch for what we cannot date).
  * ``consistency``  â€” keyword homogeneity across the theme's papers, a proxy for
...
from lfx_insights.corpus_features import max_year as corpus_max_year
from lfx_insights.corpus_features import theme_papers
...
# breadth+consistency alone â€” recency is a true gate, not a mean tie-breaker.
_RECENCY_GATE = 0.5
...
def _design_for(maturity: float, recency: float, recency_known: bool) -> tuple[str, bool]:
    ...
    on their own, buy a confirmatory recommendation, and an undated corpus â€” whose
    recency we cannot vouch for â€” must not read as confirmatory-ready either.
