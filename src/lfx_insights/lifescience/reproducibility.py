...
- ``sample_size_adequacy`` (0.15): an *explicit* reported ``n`` that is also **>= 30**
  (presence of an ``n`` alone is deliberately not enough â€” that was an old-tool bug);
...
from lfx_insights.scoring.common import make_score
...
# *not* enough â€” a paper that only says "small sample (n=5)" must not score the point.
_MIN_ADEQUATE_N = 30
...
def audit_reproducibility(
    ...
    (``{paper_id: full text}``, e.g. fetched from PerspicacitÃ©) for a fair audit. Each
