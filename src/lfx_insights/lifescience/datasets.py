...
* **GEO** â€” Gene Expression Omnibus series/samples/datasets (``GSE``/``GSM``/``GDS`` ``\\d+``)
* **SRA** â€” Sequence Read Archive runs/studies and BioProjects (``SRR``/``SRP``/``PRJNA``)
* **PRIDE** â€” proteomics datasets (``PXD\\d+``)
* **MetaboLights** â€” metabolomics studies (``MTBLS\\d+``)
* **ArrayExpress** â€” functional genomics (``E-XXXX-\\d+``)
* **ENA** â€” European Nucleotide Archive projects (``PRJEB\\d+``)
* **dbGaP** â€” controlled-access studies (``phs\\d+``)
* **EGA** â€” European Genome-phenome Archive studies/datasets (``EGAS``/``EGAD`` ``\\d+``)
...
from lfx_insights.models import EvidenceRef, Insight
...
def discover_datasets(corpus: Corpus, full_texts: dict[str, str] | None = None) -> list[Insight]:
    ...
    e.g. fetched from PerspicacitÃ©) for meaningful recall.
