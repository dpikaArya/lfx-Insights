...
* :class:`TfidfBackend` â€” a generic lexical retriever over a *fixed* candidate
  pool (a vanilla TF-IDF baseline, deliberately **not** PerspicacitÃ©).
* :class:`OracleBackend` â€” perfect retrieval: returns a case's own gold contexts.
...
if TYPE_CHECKING:
    ...
    from lfx_insights.sources.base import RetrievalBackend


class NullBackend:
    """Closed-book backend: retrieves nothing.

    Establishes the floor condition for the ablation â€” answers are generated with
...
class OracleBackend:
    ...
    the right papers. The ablation ceiling â€” it decouples generation/citation quality
...
def build_eval_backend(
    ...
    *,
    ...
    url, timeout:
        PerspicacitÃ© MCP endpoint settings (used only for ``"perspicacite"``).
