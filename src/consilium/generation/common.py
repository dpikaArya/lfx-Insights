...
def disambiguation_suffixes(papers: list[Paper]) -> dict[str, str]:
    ...
    they are ordered by title (APA 7th, §9.47) and assigned ``a``, ``b``, ``c`` ...
    ...
    citations and the reference list — guaranteeing they always agree.
    """
    unique: list[Paper] = []
    seen: set[str] = set()
    for p in papers:
...
def has_output_leak(text: str) -> bool:
    """Detect unformatted f-string placeholders ('{top_theme}') or 'NaN' leaks —
