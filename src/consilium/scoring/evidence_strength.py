...
shared kernel — never a bare magic number. Recency is anchored to the most recent
...
from consilium.corpus_features import max_year, theme_papers
...
def _source_diversity(theme: Theme, corpus: Corpus) -> float:
    ...
    theme drawing on one source is no worse off than a small one — adding more
...
def score_evidence_strength(themes: list[Theme], corpus: Corpus) -> list[Insight]:
    ...
    - ``source_diversity`` (weight 0.2): distinct sources normalized against
      ``_TARGET_SOURCES`` (size-independent — a large same-source theme is not
