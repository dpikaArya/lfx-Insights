...
verdict is scoped to THIS corpus — it is not a claim about the whole literature:

- *Not Supported* — already covered (high similarity).
- *Uncertain* — partially covered.
- *Confirmed* — on-topic but no close prior work in this corpus (a candidate gap).
- *Out of Scope* — similarity to the entire corpus is so low the gap is likely
...
from consilium.scoring.common import clamp01, cosine, make_score, sample_uncertainty

if TYPE_CHECKING:
    import numpy as np
    from numpy.typing import NDArray

    from consilium.models import Corpus, Insight
...
# the off-topic floor — both adapt across embedders (TF-IDF vs sentence-transformers
    ...
    *,
    ...
    for gi, gap in enumerate(gaps):
        ...
        if verdict == "Not Supported":
            ...
            reasoning += (
                "On-topic but no close prior work here — a candidate open gap; confirm "
            ...
            reasoning += (
                ...
                "OFF-TOPIC than open — verify it is on-topic before treating it as a gap "
                "(high uncertainty)."
            )
        else:
            reasoning += "Partial coverage — related work exists but does not fully address it."
