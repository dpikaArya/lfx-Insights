...
- ``emergence`` — fraction of theme papers published in the most recent window
  (``year >= maxYear - 2``), anchored to the *corpus* max year. Weight 0.4.
- ``scarcity`` — ``1 - minmax_normalize(theme size)``: small (under-explored)
  themes are more of an opportunity. Weight 0.3.
- ``momentum`` — sign-aware transform of the theme's *own* raw yearly-count
...
neutral-0.5 normalization rule (no spread → 0.5) is inherited from
...
from consilium.corpus_features import max_year, theme_papers, theme_years
