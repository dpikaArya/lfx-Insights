...
- **ScholarQABench** ``{id, subject, input, output, ctxs}`` — multi-paper long-form QA.
- **ExpertQA** (Malaviya et al., NAACL 2024) ``{question, metadata.field, answers{model->
  {answer_string, revised_answer_string, claims[{claim_string, evidence}]}}}`` — expert,
  multi-domain attributed QA.
- **LitSearch** (Ajith et al., EMNLP 2024) ``{query, corpusids, specificity, quality}`` —
  scientific literature-search *retrieval* (gold corpus ids → intrinsic recall@k/nDCG).
...
def _load_records(source: str) -> list[dict[str, Any]]:
    ...
    if source in _BUNDLED:
        resource = files("consilium.eval") / "fixtures" / _BUNDLED[source]
