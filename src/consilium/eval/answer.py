...
This is the Consilium *synthesis* half of the eval pipeline: given a question and a
...
It reuses the manuscript grounding pattern — a numbered corpus listing fed to a
...
Consilium's anti-hallucination discipline applied to the eval answerer: an answer
...
from consilium.eval.models import Citation, GeneratedAnswer, RetrievedDoc
from consilium.models import Provenance
from consilium.standards.grounding import verify_quote_in

if TYPE_CHECKING:
    from consilium.eval.entailment import Entailer
...
def _source_listing(docs: list[RetrievedDoc]) -> str:
    ...
    supply a verbatim supporting quote — a title-only listing would force every
...
def _selfground(text: str, docs: list[RetrievedDoc], entailer: Entailer) -> tuple[str, list[int]]:
    ...
    1. **Gate** (precision): drop a ``[k]`` whose ``docs[k]`` does not entail its sentence —
    ...
    2. **Attribute** (recall, "cite-as-you-write"): a scorable sentence left with no
       citation is given ``[d]`` for the first source that entails it — so supported claims
...
def answer_question(
    ...
    *,
    ...
    return GeneratedAnswer(
        ...
        docs=docs,
        citations=citations,
        provenance=Provenance(generated_by="consilium", model="(eval)"),
    )
