...
* **recall** — fraction of scored sentences whose joined cited passages
  entail the sentence (citation *recall*: is the claim supported?);
* **precision** — fraction of individual citations that are not redundant
  (citation *precision*: each marker is either sufficient on its own or
  necessary to the joint support — over-cited markers are penalised);
...
from consilium.eval.models import CitationScore
...
# Sentences shorter than this (after stripping markers) are not scored — they are
