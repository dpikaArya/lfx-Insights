...
uncertainty) — never bare means — and the headline ``lift`` isolates Perspicacité's
...
from consilium.eval.answer import answer_question
from consilium.eval.dataset import candidate_pool
from consilium.eval.entailment import build_entailer
...
from consilium.eval.metrics.quality import judge_quality
from consilium.eval.metrics.retrieval import score_retrieval
from consilium.eval.models import (
    AblationReport,
    CaseResult,
    CitationScore,
    ConditionReport,
...
from consilium.eval.retrieval import build_eval_backend
from consilium.models import Provenance, Score, ScoreComponent
from consilium.scoring.common import band, clamp01, make_score, sample_uncertainty

if TYPE_CHECKING:
    from consilium.config import Settings
    from consilium.eval.models import EvalCase
...
_PERSP_CAVEAT = (
    "Perspicacité retrieves over a different corpus than peS2o(45M); only the "
...
def run_ablation(
    ...
    *,
    ...
    return AblationReport(
        ...
        lift=lift,
        caveats=caveats,
        provenance=Provenance(generated_by="consilium.eval", model=settings.llm.model),
    )
