"""Perspicacité MCP adapter — Consilium's only literature backend.
...
from consilium.models import Author, Corpus, Paper, Passage
...
class PerspicaciteBackend:
    """Calls a running Perspicacité MCP server over streamable-HTTP."""
    ...
    def _post(self, payload: dict[str, Any]) -> httpx.Response:
        ...
        try:
            ...
            raise PerspicaciteUnavailable(
                f"Perspicacité is not reachable at {self.url}. Start the MCP server "
                "and retry. Consilium does not fall back to home-grown search."
            ) from exc
        except httpx.HTTPError as exc:
            raise PerspicaciteUnavailable(f"Perspicacité HTTP error at {self.url}: {exc}") from exc
        return resp
    ...
    def _call_tool(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if "error" in msg:
            raise PerspicaciteUnavailable(f"Perspicacité tool '{name}' failed: {msg['error']}")
        result = msg.get("result", {})
        if isinstance(result, dict) and result.get("isError"):
            raise PerspicaciteUnavailable(f"Perspicacité tool '{name}' returned an error.")
        ...
        if isinstance(payload, dict) and (payload.get("success") is False or payload.get("error")):
            ...
            raise PerspicaciteUnavailable(f"Perspicacité tool '{name}' failed: {detail}")
        return payload
