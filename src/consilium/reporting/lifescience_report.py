...
if TYPE_CHECKING:
    from consilium.models import Protocol, StatRecommendation


def render_stat(rec: StatRecommendation) -> str:
    lines = [
        f"# Statistical Recommendation — {rec.method}",
        "",
        f"- Design: {rec.design}",
...
def render_protocol(protocol: Protocol) -> str:
    lines = [f"# Protocol — {protocol.name}", "", f"_Kind: {protocol.kind}_", "", "## Steps", ""]
