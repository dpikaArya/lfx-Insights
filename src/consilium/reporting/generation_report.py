...
from consilium.generation.common import disambiguation_suffixes, format_apa
...
def render_hypotheses(hypotheses: list[Hypothesis]) -> str:
    ...
    for h in hypotheses:
        ...
        lines.append("")
        lines.append(
            f"- Relation (Bucur SuperPattern): `{h.subject}` — **{h.qualifier}** — `{h.object}`"
        )
        if h.independent_var or h.dependent_var:
            lines.append(f"- IV → DV: {h.independent_var or '?'} → {h.dependent_var or '?'}")
        if h.methodology:
...
def render_questions(questions: list[ResearchQuestion]) -> str:
    ...
    for q in questions:
        ...
        if q.score is not None:
            ...
            lines.append(f"- Score: **{q.score.value:.2f}** ({q.score.interpretation}) — {comps}")
        if q.rationale:
