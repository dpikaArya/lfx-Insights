...
from consilium.llm.client import LLMClient, MockLLM
...
from consilium.reporting.docx_export import write_docx
from consilium.reporting.lifescience_report import render_protocol, render_stat


@click.group()
def main() -> None:
    """Consilium — research strategy & authoring copilot (layered on Perspicacité)."""


@main.command()
def version() -> None:
    """Print the Consilium version."""
...
def serve(transport: str, host: str, port: int, offline: bool) -> None:
    """Run Consilium as an MCP server (requires the `mcp` extra)."""
...
def eval_scholarqa(
    ...
    """Run the Perspicacité→Consilium retrieval ablation on a ScholarQABench dataset."""
    ...
    if offline:
        ...
        from consilium.llm.client import build_client, validate_ollama
