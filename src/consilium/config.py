...
class EmbeddingSettings(BaseModel):
    ...
    # via LiteLLM (needs the provider API key).  See consilium.themes.discover.default_embedder.
    model: str = "all-MiniLM-L6-v2"


class EvalSettings(BaseModel):
    """ScholarQABench eval harness knobs (see consilium.eval)."""
    ...
    # supported). CRAG/Self-RAG + ALCE-style. See consilium.eval.answer._selfground.
...
class PipelineSettings(BaseModel):
    quick: list[str] = Field(default_factory=lambda: [*_SCORING_STAGES, *_AGGREGATION_STAGES])
    life_science: list[str] = Field(
        default_factory=lambda: [*_LIFE_SCIENCE_STAGES, *_AGGREGATION_STAGES]
    )
