...
def _estimate_clusters(
    ...
    for k in range(MIN_CLUSTERS, max_k + 1):
        ...
        if n_unique < 2:
            trial_log.append({
                "k": k, "silhouette": None,
                "reason": "fewer than 2 non-empty clusters",
            })
            continue
...
def _save_consensus_outputs(
    ...
    for ct in sorted_themes:
        ...
        report_lines.append(
            f"| {label_esc} | {count} | {conf_pct} "
            f"| {methods_str} | {kw_str} | {expl} |"
        )
    ...
    for rank, ct in enumerate(sorted_themes, 1):
        ...
        report_lines.append(
            f"**{rank}. {ct['label']}** — "
            f"Confidence: {conf_pct}, Papers: {count}, "
    ...
    report_lines.append("")
    report_lines.append(
        "The following themes are marked as exploratory "
        "(identified by only one method):"
    )
    report_lines.append("")
    ...
    if exploratory:
        ...
        report_lines.append(
            "All themes were identified by multiple methods, "
    ...
    methods_results = [
        {
            "name": "nmf",
            "labels": labels_nmf,
            "theme_labels": themes_nmf,
            "keywords": kw_nmf,
        },
        {
            "name": "hierarchical",
            "labels": labels_hier,
            "theme_labels": themes_hier,
            "keywords": kw_hier,
        },
        {
            "name": "fixed4",
            "labels": labels_fixed,
            "theme_labels": themes_fixed,
            "keywords": kw_fixed,
        },
    ]
